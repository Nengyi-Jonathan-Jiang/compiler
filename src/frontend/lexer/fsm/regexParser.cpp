...
namespace lexer::impl_ {
    ...
    std::ostream &operator<<(std::ostream &os, RegexParser::node_t node) {
        ...
        while (!stk.empty()) {
            ...
            if (n->isLeaf()) {
                const RegexParser::leaf_node_t leaf = RegexParser::tree_t::castToLeafNode(n);

                auto s = leaf->get_value<std::set<char>>();

                os << "[" << std::string{s.begin(), s.end()} << "]" << "\n";
            } else {
                const RegexParser::internal_node_t as_internal = RegexParser::tree_t::castToInternalNode(n);
                os << " {\n";
                stk.push({});
                typeof(stk) reverse;
